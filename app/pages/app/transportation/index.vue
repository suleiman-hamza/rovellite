...
const { data: transportData, error: fetcherror, refresh, status } = await useLazyFetch('/api/transport', {
...
<template>
  <main>
    ...
    <section v-else-if="transportData">
      <UPageGrid class="grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-y-6">
        ...
        >
          <span class="rounded-lg sm:p-2 flex items-center justify-center w-15 h-8.75 sm:w-20 sm:h-12.5 p-1">
            <NuxtImg :src="platform.logo" alt="Transportation Logo" class="object-contain" />
          </span>
