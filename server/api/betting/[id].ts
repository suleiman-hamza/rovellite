...
export default defineCachedEventHandler(async (event) => {
  ...
  const bettingProvider = await $fetch<BillerResponse>(`https://sandbox1.coralpay.com/coralpay-vas/api/packages/biller/${id}`, {
    method: 'GET',
    headers: {
  ...
  const filter = allBettingProviders.find((provider: Biller) => {
